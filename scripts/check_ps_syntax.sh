...
#
#  适用场景：Linux/WSL 上无法真正执行 PowerShell，而 PowerShell 有一类错误是
#  **解析期**的 —— 文件里一旦出现，整文件都加载不了，表现为调用处刷屏
#  CommandNotFound，真正的原因被埋掉（例如字符串里的 "$me:(OI)(CI)F" 会被
#  当成盘符变量 $me: 而导致解析失败）。
#
#  本脚本检查（静态、不执行）：
